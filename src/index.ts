...
class ElectrumClient extends Client {
	...
	constructor(port: number, host: string, protocol: string, callbacks?: Callbacks) {
		...
		this.onLogCallback = (callbacks && callbacks.onLog) ? callbacks.onLog : function(str: string) {
			console.log(str);
		};
	...
	initElectrum(electrumConfig: ElectrumConfig, persistencePolicy: PersistencePolicy = { retryPeriod: 10000, maxRetry: 1000, pingPeriod: 120000, callback: null }): Promise<ElectrumClient> {
		...
		return new Promise((resolve, reject) => {
			this.connect().then(() => {
				this.server_version(electrumConfig.client, electrumConfig.version).then((versionInfo) => {
